...
from nature import *
# import globals as G
from common_config import Globals as G
from blocks import *
...
class TerrainGeneratorSimple(TerrainGeneratorBase):
    ...
    def __init__(self, world, seed):
        ...
        self.lowlevel_ores = ((stone_block,) * 75 + (diamondore_block,) * 4 )#+ (sapphireore_block,) * 2)
        ...
        self.world_type_grass = (YFlowers, TallGrass, Rose, TallGrass0, TallGrass1, Cactus, TallGrass2, TallCactus, TallGrass3, TallGrass4, TallGrass5, TallGrass6, TallGrass7, DeadBush, DesertGrass, Reed)
        ...
        self.nether = (nether_block, soulsand_block, nether_quartz_ore_block, air_block)
    ...
    def generate_sector(self, sector):
        ...
        if sector in world.sectors:
            ...
            #     if world.world[pos] not in self.autogenerated_blocks:
            #         return

        #if G.TERRAIN_CHOICE != 'nether':
        TERRAIN_CHOICE = self.biome_generator.get_biome_type(sector[0], sector[2])
        # print("TERRAIN: ", TERRAIN_CHOICE)
        TREE_CHANCE = G.TREE_CHANCE.value
        WILDFOOD_CHANCE = G.WILDFOOD_CHANCE.value
        GRASS_CHANCE = G.GRASS_CHANCE.value
        ...
        if 0 <= by < (self.height_base + self.height_range):
            ...
            def choose(x):
                ...
                else: return self.rand.choice(x)
            ...
            if TERRAIN_CHOICE == G.NETHER:
                highlevel_ores = ((nether_block,) * 60 + (soulsand_block,) * 35 + (nether_quartz_ore_block,) * 5 + (air_block,) * 10)
            ...
            for x in range(bx, bx + 16):
                ...
                # print("generating x:",bx,"to",bx+16)

                for z in range(bz, bz + 16):
                    if by < height_base:
                        ...
                        if y <= water_level:
                            ...
                            if veget_blocks is not None:
                                # print("choose veget: ",choose(veget_blocks))
